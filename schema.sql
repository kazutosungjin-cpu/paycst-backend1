...
-- Loans. Modeled after how a real loan works: applying does NOT put money
-- in your balance. It sits `pending` until it collects LOAN_APPROVALS_NEEDED
-- (see server.js) separate, distinct admin approvals recorded in
-- loan_approvals below — only then is the principal actually disbursed.
-- A single admin decline closes the loan immediately.
...
-- One row per admin decision on a loan. The UNIQUE key is what actually
-- enforces "distinct admins" — the same admin can't approve twice to fake
-- a second signature, and re-voting is blocked outright rather than just
